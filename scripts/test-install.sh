...
do
  docker run --rm -w /root ubuntu:${UBUNTU_V} bash -c "apt-get update > /dev/null && apt-get install -y nodejs npm git > /dev/null && npm init -y > /dev/null && npm install grpc-docs > /dev/null && echo 'npm with ubuntu:${UBUNTU_V} success'"
